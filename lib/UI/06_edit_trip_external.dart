...
class _EditTripExternalState extends State<EditTripExternal> {
  ...
  void initState() {
    ...
    _isSlingExpanded = List.generate(
        widget.trip.loads.length,
        (loadIndex) => List.generate(
            widget.trip.loads[loadIndex].slings?.length ?? 0, (_) => false));
  ...
  void showBYOMandEditTripExternalSelectionDialog(
      int selectedLoadIndex, int selectedSlingIndex) async {
    ...
    bool isSelectAllCheckedGear =
        gearList.every((gear) => selectedItems.contains(gear)) &&
            selectedGearQuantities.entries
                .every((entry) => entry.value == entry.key.quantity);
    ...
    const List<String> netTypes = [
      'Cargo Net (20\'x20\')',
      'Cargo Net (12\'x12\')'
    ];

    // Check if **any net** already exists in **this sling only**
    bool netExistsInSling =
        existingSlingAccoutrements.any((acc) => netTypes.contains(acc));

    // **Filter available accoutrements for this sling**
    final List<String> availableAccoutrements =
        allLoadAccoutrements.where((acc) {
          ...
          builder: (context, dialogSetState) {
            void updateSelectAllState() {
              dialogSetState(() {
                ...
                isSelectAllCheckedGear = gearList
                        .every((gear) => selectedItems.contains(gear)) &&
                    selectedGearQuantities.entries
                        .every((entry) => entry.value == entry.key.quantity);
              });
            }
                                ...
                                children:
                                  ...
                                  return Container(
                                    ...
                                    child: CheckboxListTile(
                                      ...
                                      value: selectedItems.any((item) =>
                                          item is LoadAccoutrement &&
                                          item.name == accName),
                                                                ...
                                                                TextButton(
                                                                  ...
                                                                  child: Text(
                                                                      ...
                                                                      style: TextStyle(
                                                                          ...
                                                                          color:
                                                                              AppColors.saveButtonAllowableWeight)),
                                                                ),
                                                              ],
                                                            );
                                                          },
                    ...
                    setState(() {
                      ...
                      for (var item in selectedItems) {
                        if (item is Gear) {
                          ...
                          final int existingGearIndex = loads[selectedLoadIndex]
                                  .slings?[selectedSlingIndex]
                                  .loadGear
                                  .indexWhere(
                                    (loadItem) =>
                                        loadItem is Gear &&
                                        loadItem.name == item.name &&
                                        loadItem.isPersonalTool ==
                                            item.isPersonalTool,
                                  ) ??
        ...
        if (load.slings != null) {
          for (var sling in load.slings!) {
            for (var gear in sling.loadGear) {
              usedGearQuantities[gear.name] =
                  (usedGearQuantities[gear.name] ?? 0) + gear.quantity;
            }
          }
        }
      }
  ...
  int calculateAvailableWeight(Load load) {
    ...
    if (load.slings != null) {
      for (var sling in load.slings!) {
        ...
        totalWeight +=
            sling.loadGear.fold(0, (sum, gear) => sum + gear.totalGearWeight);
        ...
        totalWeight +=
            sling.loadAccoutrements.fold(0, (sum, acc) => sum + acc.weight);
      }
    }
  ...
  num calculateSlingWeight(Sling sling) {
    return sling.loadGear.fold(0, (sum, gear) => sum + gear.totalGearWeight) +
        ...
        sling.loadAccoutrements
            .fold(0, (sum, accoutrement) => sum + accoutrement.weight);
  }
                ...
                children: [
                  Flexible(
                    child: ReorderableListView.builder(
                      proxyDecorator: (Widget child, int loadIndex,
                      ...
                      onReorder: (oldIndex, newIndex) {
                        setState(() {
                          ...
                          final slingExpandedState =
                              _isSlingExpanded.removeAt(oldIndex);
                        ...
                        return Dismissible(
                                ...
                                return AlertDialog(
                                  ...
                                  content: Text(
                                    ...
                                    style: TextStyle(
                                        color: AppColors.textColorPrimary,
                                        fontSize:
                                            AppData.miniDialogBodyTextSize),
                                  ),
                                  actions: [
                                    TextButton(
                                      onPressed: () {
                          ...
                          onDismissed: (direction) {
                            setState(() {
                              ...
                              if (_isExpanded.length > loadIndex)
                                _isExpanded.removeAt(loadIndex);
                              if (_isSlingExpanded.length > loadIndex)
                                _isSlingExpanded.removeAt(loadIndex);
                            });
                          },
                          child: Container(
                                ...
                                GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      borderRadius: ((calculateAvailableWeight(
                                                      loads[loadIndex]) >
                                                  widget.trip.allowable) ||
                                      ...
                                      border: Border.all(
                                        ...
                                        width:
                                            0.5, // Adjust thickness as needed
                                      ),
                                    ),
                                    child: Row(
                                      mainAxisAlignment:
                                        ...
                                        Dismissible(
                                          ...
                                          onDismissed: (direction) {
                                            setState(() {
                                              ...
                                              // Remove expansion state
                                              if (_isSlingExpanded.length >
                                                      loadIndex &&
                                                  _isSlingExpanded[loadIndex]
                                                          .length >
                                                      slingIndex) {
                                                _isSlingExpanded[loadIndex]
                                                    .removeAt(slingIndex);
                                                      ...
                                                      child: Container(
                                                        child: Column(
                                                          children: [
                                                                ...
                                                                (item) =>
                                                                    Dismissible(
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        color: item
                                                                                is LoadAccoutrement
                                                                            ? AppColors.loadAccoutrementBlueGrey
                                                                            : AppColors.gearYellow,
                                                            ...
                                                            GestureDetector(
                                                              onTap: () =>
                                                                  showBYOMandEditTripExternalSelectionDialog(
                                                                      loadIndex,
                                                                      slingIndex),
                                                              child: Padding(
                                                                padding:
                                      ...
                                      child: GestureDetector(
                                        onTap: () {
                                          setState(() {
                                            ...
                                            while (_isSlingExpanded.length <=
                                                loadIndex) {
                                              _isSlingExpanded.add(
                                                  []); // Fill missing entries
                                            }

                                            _isSlingExpanded[loadIndex].add(
                                                true); // Default to expanded state
                                          });
                                        },
                                        child: Row(
                                          mainAxisAlignment:
                      ...
                      child: GestureDetector(
                        onTap: () {
                          setState(() {
                            ...
                            _isSlingExpanded.add(
                                []); // Start with an empty sling expansion list
                          });
                        },
                        child: Row(
